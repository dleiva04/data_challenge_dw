...
from pyspark.sql.functions import col, collect_list, from_json, struct, to_json, when
...
lineas_ofertadas_df = (
    spark.read.table("demo_datachallenge.bronze.lineasofertadas")
...
ofertas_df = spark.read.table("demo_datachallenge.bronze.ofertas").distinct()


# COMMAND ----------

# Load the tables
proveedores_df = spark.table("demo_datachallenge.bronze.proveedores").withColumn(
    "cedula_proveedor", col("cedula_proveedor").cast("string")
)

# COMMAND ----------

representantes_df = (
    spark.table("demo_datachallenge.bronze.representantes")
...
representantes_df = representantes_df.withColumn(
    "lista_de_representantes", to_json("lista_de_representantes")
)
...
representantes_df = representantes_df.withColumn(
    "lista_de_representantes", from_json(col("lista_de_representantes"), schema)
)

# COMMAND ----------

consorcios_df = (
    spark.table("demo_datachallenge.bronze.consorcios")
...
# Group by 'cedula_proveedor' and collect 'nro_consorcio' values into a list
consorcios_ofertas_collapse_df = consorcios_df.groupBy("nro_consorcio").agg(
    collect_list("cedula_proveedor").alias("lista_de_proveedores_del_consorcio")
)
...
proceso_ofertas_df = ofertas_df.join(
    lineas_ofertadas_df, on=["nro_oferta", "nro_sicop"], how="left"
).join(proveedores_df, on="cedula_proveedor", how="left")
...
proceso_ofertas_df = proceso_ofertas_df.join(
    representantes_df, on="cedula_proveedor", how="left"
)

# COMMAND ----------

proceso_ofertas_df = proceso_ofertas_df.join(
    consorcios_proveedor_collapse_df, on="cedula_proveedor", how="left"
)
...
proceso_ofertas_df = proceso_ofertas_df.withColumn(
    "nro_sicop", col("nro_sicop").cast("string")
)
...
proceso_ofertas_df = proceso_ofertas_df.withColumn(
    "nro_consorcio", col("nro_consorcio").cast("string")
)
...
proceso_ofertas_df.write.mode("overwrite").saveAsTable(
    "demo_datachallenge.silver.lineasofertadas"
)
